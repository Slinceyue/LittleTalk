<template>
  <AppModal @close="ui.closeModal()">
    ...
    <div class="form-group">
      <label>群名称</label>
      <input v-model="name" type="text" placeholder="请输入群名称" @keyup.enter="create" />
    </div>
    <button class="btn btn-primary w-full" @click="create" :disabled="loading">
      {{ loading ? '创建中...' : '创建' }}
    </button>
  </AppModal>
</template>
...
async function create() {
  if (!name.value.trim()) { ui.showToast('请输入群名称', 'error'); return }
  loading.value = true
  try {
    const room = await contacts.createGroup(name.value.trim())
    ui.closeModal()
    ui.showToast('创建成功', 'success')
    ui.navigateTo('chat', { type: 'group', id: room.id, name: room.name })
  } catch (e) {
